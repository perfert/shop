...
public class User {
	...
	/*OST /user/check_phone_available
...
*/
    ...
    public TokenResult checkPhone(String region, String phone) throws Exception {
        if (region == null) 
            throw new IllegalArgumentException("Paramer 'region' is required");
        if (phone == null) 
            throw new IllegalArgumentException("Paramer 'phone' is required");
        
        StringBuilder sb = new StringBuilder();
        sb.append("&region=").append(URLEncoder.encode(region.toString(), UTF8));
        sb.append("&phone=").append(URLEncoder.encode(phone.toString(), UTF8));
        String body = sb.toString();
        if (body.indexOf("&") == 0) {
            body = body.substring(1, body.length());
        }
        
        HttpURLConnection conn = HttpUtil.CreatePostHttpConnection(HostType.API, appKey, appSecret, "/user/check_phone_available", "application/x-www-form-urlencoded");
