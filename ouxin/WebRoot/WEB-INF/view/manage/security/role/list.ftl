...
<@mngTpl>	
	...
	<body class="no-skin">
		<@container>
			<@breadcrumbs items="角色列表" />			
			<@content>
				<@dataTable pager=pager prefix="${ctxPath}/manage/security/${systemModuleId!}/role/"; pager, prefix>
					...
					<@thead>
						...
						<th class="sorting_disabled" style="width:20%;">备注</th>
					...
					<@tbody pager=pager prefix=prefix; item, pager, prefix>
						<tr role="row" class="odd ${(1==item.state)?string('', ((0==item.state)?string('light-orange', 'light-grey')))}">
							...
							<@operator item=item prefix=prefix width="10">
								<a class="blue" href="${prefix!}${item.id}/permission/list"> <i class="ace-icon fa btn-link bigger-130"></i>权限配置</a>
							</@operator>
						</tr>
					</@tbody>
