			...
			<@content>
				<@dataTable pager=pager prefix="${ctxPath}/manage/store/shops/" formId="memberListForm"; pager, prefix>
					...
					<@thead>
						...
						<th class="sorting_disabled" style="width:10%;">店铺地址</th>
						<th class="sorting_disabled" style="width:10%;">审核状态</th>
						<th class="sorting_disabled" style="width:10%;">店铺状态</th>
					...
					<@tbody pager=pager prefix=prefix; item, pager, prefix>
						<tr role="row" class="odd ${(1==item.state)?string('', ((0==item.state)?string('light-orange', 'light-grey')))}">
							...
							<td style="width:10%;"><#if 0 lte item.authStatus>通过<#else>未审核</#if></td>
							<td style="width:10%;">
							...
							<td class="op" style="width:5%;">
								<div class="hidden-sm hidden-xs action-buttons">
									<#if 0 lte item.state>
										<a class="green" href="${ctxPath!}/manage/store/certificate/verify/${(item!).id!}"> <i class="ace-icon glyphicon glyphicon-lock bigger-130"></i></a>
									</#if>
								</div>
							</td>
						</tr>
