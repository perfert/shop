...
<@mngTpl>	
	...
	<body class="no-skin">
		<@container>
			<@breadcrumbs items="好友申请列表" />			
			<@content>
				<@dataTable pager=pager prefix="${ctxPath}/manage/chat/apply/"; pager, prefix>
					<div class="row">
						<div class="col-sm-8" style="width:auto;">
							<div class="dataTables_filter">
								<label>申请账号：<input type="text" name="query.applyAccount" value="${query.applyAccount}" aria-controls="sample-table-2" /></label>
								<label>被申请账号：<input type="text" name="query.receiveAccount" value="${query.receiveAccount}" aria-controls="sample-table-2" /></label>
					...
					<@thead>
						...
						<th class="sorting_disabled" style="width:20%;">加好友账号</th>
						<th class="sorting_disabled" style="width:15%;">申请状态</th>
						<th class="sorting_disabled" style="width:20%;">请求消息</th>
					...
					<@tbody pager=pager prefix=prefix; item, pager, prefix>
						<tr role="row" class="odd ${(1==item.state)?string('', ((0==item.state)?string('light-orange', 'light-grey')))}">
							...
							<td style="width:20%;">${item.applyAccount}[${item.applyNickName}]</td>
							<td style="width:20%;">${item.receiveAccount}[${item.receiveNickName}]</td>
							<td style="width:15%;">${item.type}</td>
							<td style="width:20%;">${item.msg}</td>
							<td style="width:5%;">
							...
							<td class="op" style="width:10%;">
								<div class="hidden-sm hidden-xs action-buttons">
									<#if 0 lte item.state>
									<a class="delete-dialog red" href="javascript:void(0);" link="${ctxPath}/manage/chat/apply/del/${item.id}" > <i class="ace-icon fa fa-trash-o bigger-130"></i></a>
									</#if>
								</div>
							</td>
						</tr>
