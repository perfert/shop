...
<@mngTpl>	
	...
	<body class="no-skin">
		<@container>
			<@breadcrumbs items="财富类型列表" />			
			<@content>
				<@dataTable pager=pager prefix="${ctxPath}/manage/wallet/wealth_type/"; pager, prefix>
					...
					<@thead>
						...
						<th class="sorting_disabled" style="width:30%;">类型名称</th>
						<th class="sorting_disabled" style="width:15%;">初始值</th>
						<th class="sorting_disabled" style="width:5%;">是否默认</th>
					...
					<@tbody pager=pager prefix=prefix; item, pager, prefix>
						<tr role="row" class="odd ${(1==item.state)?string('', ((0==item.state)?string('light-orange', 'light-grey')))}">
							...
							<td style="width:15%;">${item.init}</td>
							<td style="width:5%;">
								<#if item.isDefault>
								   <span class="ace-icon fa green"><i class="fa-check"></i></span>
								<#else>
								   <span class="ace-icon fa light-orange"><i class="fa-times"></i></span>
								</#if>
							</td>
