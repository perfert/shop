...
<@mngTpl>	
	...
	<body class="no-skin">
		<@container>
			<@breadcrumbs items="点赞列表" />			
			<@content>
				<@dataTable pager=pager prefix="${ctxPath}/manage/news/${article}/good/"; pager, prefix>
					<div class="row">
						<div class="col-sm-8" style="width:auto;">
							<div class="dataTables_filter">
								<label>点赞账号：<input type="text" name="query.account" value="${query.account}" aria-controls="sample-table-2" /></label>
					...
					<@thead>
						...
						<th class="sorting_disabled" style="width:35%;">点赞状态</th>
					...
					<@tbody pager=pager prefix=prefix; item, pager, prefix>
						<tr role="row" class="odd ${(1==item.state)?string('', ((0==item.state)?string('light-orange', 'light-grey')))}">
							...
							<td style="width:35%;"><#if item.goodState == "1">点赞<#else>取消</#if></td>
